...
const sampleMail1 = async (from, subject, destination, NotifyData, type) => {
  ...
  const sendPartnerWelcomeMessage = async (id, type, title) => {
    ...
    if (type == "retryText") {
      ...
      let queryData =  await conn.query(textDetails);
      let getTextMsgData =queryData.rows;
    ...
    const sqlQuery = `
      ...
      JSON_BUILD_OBJECT(
          ...
          'artist_id', message_recipient.artist_id,
          'manual', message_recipient.manual,
          'enquiry_artist_id', message_recipient.enquiry_artist_id,
          'artist', JSON_BUILD_OBJECT(
              'email', artist.email,
              'artist_id', artist.artist_id,
              'mobile', artist.mobile
          )
      ) AS message_recipients
      FROM
          message
      INNER JOIN
          message_recipient ON message.message_id = message_recipient.message_id
      INNER JOIN
          artist ON message_recipient.artist_id = artist.artist_id
      WHERE
          message_recipient.message_id = ${id}`;
      const a = await conn.query(sqlQuery)
      console.log(a.rows,'query');
         messageData = a.rows;
    ...
    if (type == "text") {
      ...
      if (messageData.length > 0) {
        for (let i = 0; i < messageData.length; i++) {
          ...
          if (numberssArr.length > 0) {
            console.log('hello from type text')
            const params = new URLSearchParams();
            params.append("apikey", apiKey);
            params.append("sender", "MadDog");
            params.append("numbers", numberssArr);
